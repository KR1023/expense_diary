...
- 통계 탭은 중간 메뉴 없이 `지출 통계` 화면으로 바로 진입합니다.
- 통계 화면은 월 요약, 일평균, 전월 비교, 최대 지출일, 월 지출 추세, 일별 지출 분포, 분류별 breakdown, 결제 수단별 breakdown/detail sheet를 포함합니다.
- 통계 화면 우측 상단의 내보내기 버튼은 CSV/PDF 보고서 내보내기 화면으로 이동합니다. 내보내기 화면에서 기간과 포맷을 선택해 생성/공유합니다.
- 월 지출 추세 차트는 선택 월을 기준으로 3/6/12개월 범위를 보여줍니다. 막대 클릭 시 통계 기준 월은 이동하지 않고, 하단 일별 지출 분포의 표시 월만 변경합니다.
- 통계 차트의 KRW 값은 `(만원)` 단위 기준으로 막대 위에는 숫자만 표시합니다. USD는 `(USD)` 단위 기준으로 숫자만 표시합니다.
- 통계 차트 막대 색상은 설정의 배경 선택 테마를 따라야 합니다. `AppColors.heroGradientForBackground()`와 `accentColorForBackground()`를 기준으로 색을 계산합니다.
- 통계 화면의 일별/분류별/결제수단별 상세 드로어는 지출 내역 탭의 일자 상세 드로어 스타일을 기준으로 합니다. 드로어 배경, 헤더 gradient, 내부 카드/테두리 색은 현재 배경 선택 테마에 맞춰 변경되어야 합니다.
