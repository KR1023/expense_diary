...
- (id _Nullable)readValueOfType:(UInt8)type {
  ...
  case FLTAdmobRequestConfigurationParams: {
    ...
    id childDirectedTag = [self readValueOfType:[self readByte]];
    if (childDirectedTag != nil && childDirectedTag != (id)[NSNull null]) {
      [requestConfig setValue:childDirectedTag
                       forKey:@"tagForChildDirectedTreatment"];
    }
    id underAgeTag = [self readValueOfType:[self readByte]];
    if (underAgeTag != nil && underAgeTag != (id)[NSNull null]) {
      [requestConfig setValue:underAgeTag forKey:@"tagForUnderAgeOfConsent"];
    }
