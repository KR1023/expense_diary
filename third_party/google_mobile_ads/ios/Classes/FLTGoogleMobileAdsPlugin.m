...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  if ([call.method isEqualToString:@"MobileAds#initialize"]) {
    ...
    @try {
      [requestConfig setValue:@(isEnabled.boolValue) forKey:@"sameAppKeyEnabled"];
    } @catch (NSException *exception) {
      // Newer SDKs may remove this API. Ignore to keep compatibility.
    }
    result(nil);
    ...
    @try {
      sdkVersion = [GADMobileAds.sharedInstance valueForKey:@"sdkVersion"];
    } @catch (NSException *exception) {
      sdkVersion = nil;
    }
    result(sdkVersion);
    ...
    if (tagForUnderAgeOfConsent != NULL &&
      ...
      case 0:
        [GADMobileAds.sharedInstance.requestConfiguration
            setValue:@(NO)
             forKey:@"tagForUnderAgeOfConsent"];
        break;
      case 1:
        [GADMobileAds.sharedInstance.requestConfiguration
            setValue:@(YES)
